...
export const GiftPage = ({ extraClass = "" }) => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("auth_token")) {
      Promise.allSettled([getCard(id), getOwnWishes()]).then(
        ([card, wishlist]) => {
          console.log(card, wishlist);
          card.status === "fulfilled" && setWishData(card.value);
          if (wishlist.status === "fulfilled") {
            const goodInMyWish = wishlist?.value?.find(
              (item) => item.id === card.value.id
            );
            console.log(goodInMyWish);
            if (goodInMyWish) {
